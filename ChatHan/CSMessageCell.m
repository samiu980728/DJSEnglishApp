...
- (void)setMessageModel:(DJSChatMessageModel *)messageModel {
                          ...
                          bgView:(UIView *)supView
{
    ...
    DJSChatMessageModel *messageModel=self.messageModel;
    ...
    //setTargetRect: 指向的矩形框 targetView:targetRect以targetView的左上角为坐标原点
...
#pragma mark 剪切板代理方法 必须要得通过第一响应者，来告诉MenuController它内部应该显示什么内容 前提是：必须要有第一响应者，让第一响应者，实现上面方法，告诉显示什么内容。实现下面方法，可以让某个视图或者控制器，成为第一响应者： canBecomeFirstResponder方法。
