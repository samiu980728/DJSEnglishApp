...
@interface DJSChatViewController ()<UITextViewDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,CSMessageCellDelegate, EmojiViewDelegate,UITableViewDataSource,UITableViewDelegate>
...
@implementation DJSChatViewController
...
//    self.edgesForExtendedLayout = UIRectEjpxdgeNone;
    
    _dataArray = [NSMutableArray arrayWithCapacity:0];
    
    
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 20, self.view.frame.size.width, self.view.frame.size.height-44) style:UITableViewStyleGrouped];
    ...
    DJSChatMessageModel *model = [[DJSChatMessageModel alloc] init];
    ...
    model = [[DJSChatMessageModel alloc] init];
    model.showMessageTime=YES;
...
{
    DJSChatMessageModel *model = _dataArray[indexPath.row];
...
- (void)messageCellSingleClickedWith:(CSMessageCell *)cell
{
    ...
    {
        [[DJSRecord ShareCSRecord] playRecord];
...
- (void)touchDown:(UIButton *)btn
{
    if (iiii == 0)
    {
        [[DJSRecord ShareCSRecord] beginRecord];
        iiii = 1;
    }
...
- (void)leaveBtnClicked:(UIButton *)btn
{
    iiii = 0;
    NSLog(@"松开了");
    [[DJSRecord ShareCSRecord] endRecord];
}
